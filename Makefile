GOPKG_BASE := github.com/raravena80/gotestsshd
...
build: workdir/gotestsshd

build-native: $(GOFILES)
	go build $(GO_LDFLAGS) -o workdir/gotestsshd .

build-static: $(GOFILES)
	go build $(GO_LDFLAGS_STATIC) -o workdir/gotestsshd .

workdir/gotestsshd: $(GOFILES)
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build $(GO_LDFLAGS) -o workdir/gotestsshd.linux.amd64 .
